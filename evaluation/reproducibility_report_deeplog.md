# Reproducibility Report: DeepLog

This document records the metadata required for reproducing the experimental results of DeepLog on the HDFS dataset.
...
| Parameter | DeepLog |
| :--- | :--- |
| **Model Type** | LSTM (Seq2Seq approach) |
| **Input Size** | 1 (Index) |
| **Hidden Size** | 64 |
| **Num Layers** | 2 |
| **Optimizer** | Adam |
...
3.  **Sessionizaton**: Group by `BlockId`. Filtering `len >= window_size`.
4.  **Indexing**: `IntId` - 1 = `Model Input ID` (0-based).
