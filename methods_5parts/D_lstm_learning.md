...
- 追加：IAE,ISE,ITAEを適用することが本題なので、それらの計算のために、以下を行う。
- 手順
  1. 予測: モデルは全ログキーに対する確率分布を出力。
     例: [キーA: 0.1, キーB: 0.8, キーC: 0.1]
  2. 観測: 次に実際に「キーA」が来たとする。
  3. 連続値への変換:モデルはキーAに対して「0.1（10%）」しか予測していない。この「実際の正解に対する予測確率」を使う。
  4. 瞬時誤差 $e(t)$の定義:$$e(t) = 1.0 - (\text{実際に来たキーの予測確率})$$この場合、 $e(t) = 1.0 - 0.1 = 0.9$ （大きな誤差＝異常）もしキーBが来ていたら、 $e(t) = 1.0 - 0.8 = 0.2$ （小さな誤差＝正常に近い）
  5. 結果：これにより、離散的なログキーの羅列が、$0.0$〜$1.0$の間を変動する連続的な誤差信号（波形）に変換される。これで物理量と同じように扱えるようになる。

以上の処理を行い、それにより生まれた波形をプロットする。また、IAE等々を計算する。
